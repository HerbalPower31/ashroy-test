...
const ManageAdmins = () => {
  ...
  const handleViewCalendar = (admin) => {
    console.log('Viewing calendar for:', admin.name);
    console.log('Available rooms:', rooms.filter(room => room.owner === admin.name));
    console.log('Available bookings:', bookings[admin.name]);
        ...
        {viewingCalendar && (
          <>
            ...
            <DialogContent>
              <Box sx={{ mb: 3 }}>
                <Typography variant="body2" color="text.secondary">
                  View and manage bookings for all rooms. Click on a booking to see details.
                </Typography>
              </Box>
              {rooms
                .filter(room => room.owner === viewingCalendar.name)
                .map(room => {
                  const roomBookings = bookings[viewingCalendar.name]?.filter(b => b.roomId === room.id) || [];
                  return (
                    <Box key={room.id} sx={{ mb: 4 }}>
                      <Paper elevation={3} sx={{ p: 2, mb: 2 }}>
                        ...
                        <Typography variant="body2" color="text.secondary" gutterBottom>
                          Capacity: {room.capacity} guests | Size: {room.size}
                        </Typography>
                      </Paper>
                      <RoomCalendar
                        room={room}
              ...
              {rooms.filter(room => room.owner === viewingCalendar.name).length === 0 && (
                <Paper sx={{ p: 4, textAlign: 'center' }}>
                  <Typography variant="h6" color="text.secondary" gutterBottom>
                    No Rooms Found
                  </Typography>
                  <Typography variant="body1" color="text.secondary">
                    This partner hasn't added any rooms yet.
                  </Typography>
                </Paper>
              )}
            </DialogContent>
